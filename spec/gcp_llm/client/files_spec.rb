  ...
  describe "#files", :vcr do
    ...
    let(:upload) do
      VCR.use_cassette(upload_cassette) do
        GcpLlm::Client.new.files.upload(parameters: { file: file, purpose: upload_purpose })
      end
    end
    ...
    describe "#list" do
      ...
      let(:response) { GcpLlm::Client.new.files.list }
    ...
    describe "#retrieve" do
      ...
      let(:upload_cassette) { "#{cassette} upload" }
      let(:response) { GcpLlm::Client.new.files.retrieve(id: upload_id) }
    ...
    describe "#content" do
      ...
      let(:upload_cassette) { "#{cassette} upload" }
      let(:response) { GcpLlm::Client.new.files.content(id: upload_id) }
    ...
    describe "#delete" do
      ...
      let(:response) do
        GcpLlm::Client.new.files.delete(id: upload_id)
      end

      before do
        # We need to check the file has been processed by GcpLlm
        # before we can delete it.
        retrieved = VCR.use_cassette(retrieve_cassette) do
          GcpLlm::Client.new.files.retrieve(id: upload_id)
        end
        tries = 0
